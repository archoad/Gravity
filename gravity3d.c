...
void update(int value) {
	int i=0;
	double limit=25.0, dtms=0.0, friction=0.85, gravity=9.81;
	...
	pathLength ++;
	for (i=0; i<value; i++) {
		planetsList[i].pos.x += planetsList[i].velocity.x;
		planetsList[i].pos.y += planetsList[i].velocity.y;

		if ((planetsList[i].pos.z <= -limit + planetsList[i].radius) & (planetsList[i].velocity.z <= planetsList[i].radius)) {
			planetsList[i].pos.z = -limit + planetsList[i].radius;
			planetsList[i].velocity.x *= friction;
			planetsList[i].velocity.y *= friction;
		} else {
			planetsList[i].velocity.z -= (gravity * planetsList[i].mass * dtms);
			planetsList[i].pos.z += planetsList[i].velocity.z * dtms;
		}

		if ((planetsList[i].pos.x > (limit-planetsList[i].radius)) | (planetsList[i].pos.x < (-limit+planetsList[i].radius))) {
			planetsList[i].velocity.x = -planetsList[i].velocity.x;
		}
		if ((planetsList[i].pos.y > (limit-planetsList[i].radius)) | (planetsList[i].pos.y < (-limit+planetsList[i].radius))) {
			planetsList[i].velocity.y = -planetsList[i].velocity.y;
		}
		if ((planetsList[i].pos.z > (limit-planetsList[i].radius)) | (planetsList[i].pos.z < (-limit+planetsList[i].radius))) {
			planetsList[i].velocity.z = -planetsList[i].velocity.z;
		}

		planetsList[i].path = realloc(planetsList[i].path, (pathLength+1)*sizeof(vector));
...
void populatePlanets(void) {
	...
	for (i=0; i<sampleSize; i++) {
		...
		planetsList[i].velocity.x = generateFloatRandom() / 10.0;
		planetsList[i].velocity.y = generateFloatRandom() / 10.0;
		planetsList[i].velocity.z = 0;
